...
def ring_double_check(ring_a_lst, SCF_mol):
        ring_a_lst_check=[]
...
def detect(ac, ac_i, SCF_mol, order):
    if order == 0: order = ""
    ...
    print('ring_a_lst_2st',ring_a_lst_2nd)
    print('ring_a_lst_3st',ring_a_lst_3rd)
    ...
    #    ring_a_lst=ring_double_check(ring_a_lst, SCF_mol)
    ...
    for ra_i in ring_a_lst:
        ...
        if ra_nob==3:
            # Get the positions
            if ra_i in ring_a_lst_1st: position ='o' + str(order)
            elif ra_i in ring_a_lst_2nd: position ='m' + str(order)
            elif ra_i in ring_a_lst_3rd: position ='p' + str(order)
            else: continue
...
def position_detection(SCF_smiles_input):
    ...
    if ac.GetIsAromatic():
        sub, pos = detect(ac, ac_i, SCF_mol, 0)
        sub_s = '.'.join(sub)
        pos_s = '.'.join(pos)
        return sub_s, pos_s
    ...
    for b1 in ac.GetBonds(): # C in ortho positions
        ...
        if cons_archeck and cons_i not in cons_lst:
            ...
            sub_temp, pos_temp = detect(cons, cons_i, SCF_mol, order)
            order+=1
...
if __name__ == '__main__':
    ...
    data_df.to_excel('outbenzylic.xlsx',index=None)