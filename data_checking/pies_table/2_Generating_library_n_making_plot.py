...
clusters_coord_count.to_excel('2_cluster_coord.xlsx')
...
axs.yaxis.grid(color='gainsboro', linestyle='dotted')
axs.xaxis.grid(color='gainsboro', linestyle='dotted')
...
# =============================================================================
# # DEBUG: CHOOSE A SPECIFC CASE:
#     if FB != 'Alkyl-Aryl Cross Coupling Csp3-Csp2' or \
#     LG != 'N-N (PC)':
#         continue
# =============================================================================
...
# FIRST CASE: Spliting pie chart
    ...
    #
        ...
        # For crossc coupling, this should be equal
        sum_all1=sum(nor_lst1)+top_pair_nor
        sum_all2=sum(nor_lst2)+top_pair_nor
        print('sumall1',sum_all1,'sum_all2',sum_all2)
        sum_all=sum_all1+sum_all2
        # top_pair_nor1 and 2 are euqal but their ratio is different.
        top_pair_norp1=top_pair_nor/sum_all1/2
        top_pair_norp2=top_pair_nor/sum_all2/2
        print('top_pair_nor1',top_pair_nor,'top_pair_nor1',top_pair_nor)
        print('top_pair_norp1',top_pair_norp1,'top_pair_norp2',top_pair_norp2)
    ...
    # # create 2 pie for top_pair
        ...
        # /2 because one scaffold represent by half circle
        xy1 = np.row_stack([[0, 0], np.column_stack([x1, y1])])
        axs.scatter(LGC+0.05,FBC+0.1,s=(50+(sum_all/max_total_count)*(500)),\
        c=get_color(TP_A2),marker=xy1,edgecolor='black',linewidths=0.8)
        x1 = np.sin(2 * np.pi * np.linspace(1-top_pair_norp1, 1))
        y1 = np.cos(2 * np.pi * np.linspace(1-top_pair_norp1, 1))
        xy1 = np.row_stack([[0, 0], np.column_stack([x1, y1])])
        axs.scatter(LGC-0.05,FBC+0.1,s=(50+(sum_all/max_total_count)*(500)),\
        c=get_color(TP_A1),marker=xy1,edgecolor='black',linewidths=0.8)
    # Draw the last 2 line
        x1 = np.sin(2 * np.pi * np.linspace(top_pair_norp2, top_pair_norp2))
        y1 = np.cos(2 * np.pi * np.linspace(top_pair_norp2, top_pair_norp2))
        # /2 because one scaffold represent by half circle
        xy1 = np.row_stack([[0, 0], np.column_stack([x1, y1])])
        axs.scatter(LGC+0.05,FBC+0.1,s=(50+(sum_all/max_total_count)*(400)),\
        c=get_color(TP_A2),marker=xy1,edgecolor='black',linewidths=0.8)
        x1 = np.sin(2 * np.pi * np.linspace(1, 1))
        y1 = np.cos(2 * np.pi * np.linspace(1, 1))
        xy1 = np.row_stack([[0, 0], np.column_stack([x1, y1])])
        axs.scatter(LGC-0.05,FBC+0.1,s=(50+(sum_all/max_total_count)*(400)),\
        c=get_color(TP_A1),marker=xy1,edgecolor='black',linewidths=0.8)
#=============================================================================
    # LEFT SIDE Draw a big pie first
        prev=0
        x1 = np.sin(2 * np.pi * np.linspace(0.5, 1-top_pair_norp1))
        y1 = np.cos(2 * np.pi * np.linspace(0.5, 1-top_pair_norp1))
        xy1 = np.row_stack([[0, 0], np.column_stack([x1, y1])])
        try:
            EG=unique_lst1[0]
        except:
            continue
        axs.scatter(LGC-0.05,FBC,s=(50+(sum_all/max_total_count)*(500)), \
        c=get_color(EG),marker=xy1,linewidths=0.5)
        os.mkdir(cluster_path+'/EG1')
        os.mkdir(cluster_path+'/EG2')
    # # LEFT SIDE Draw for the rest
        for index,EG in enumerate(unique_lst1):
        # Library generating part
            EG1_path=cluster_path+'/EG1/'+EG
            os.mkdir(EG1_path)
            library_generating(cluster_df,EG,EG1_path,reactions_folder,mech_lst)
        # Plotting part
            if index==0:continue
            x1 = np.sin(2 * np.pi * np.linspace(0.5+norp_lst1[index-1]+prev,1-top_pair_norp1))
            y1 = np.cos(2 * np.pi * np.linspace(0.5+norp_lst1[index-1]+prev,1-top_pair_norp1))
            xy1 = np.row_stack([[0, 0], np.column_stack([x1, y1])])
            axs.scatter(LGC-0.05,FBC,s=(50+(sum_all/max_total_count)*(500)), \
            c=get_color(EG),marker=xy1,linewidths=0.5)
            prev=prev+norp_lst1[index-1]
            #if index == 6: break

    # RIGHT SIDE Draw a big pie first
        prev=0 #<- this is value for the upper limit
        x2 = np.sin(2 * np.pi * np.linspace(top_pair_norp2, 0.5))
        y2 = np.cos(2 * np.pi * np.linspace(top_pair_norp2, 0.5))
        ...
        try:
            ...
            continue
        axs.scatter(LGC+0.05,FBC,s=(50+(sum_all/max_total_count)*(500)), \
        c=get_color(EG),marker=xy2,linewidths=0.5)
    # RIGHT SIDE Draw for the rest
        for index,EG in enumerate(unique_lst2):
        # Library generating part
            EG2_path=cluster_path+'/EG2/'+EG
            os.mkdir(EG2_path)
            library_generating(cluster_df,EG,EG2_path,reactions_folder,mech_lst)
        # Plotting part
            if index==0:continue
            x2 = np.sin(2 * np.pi * np.linspace(top_pair_norp2, 0.5-norp_lst2[index-1]-prev))
            y2 = np.cos(2 * np.pi * np.linspace(top_pair_norp2, 0.5-norp_lst2[index-1]-prev))
            xy2 = np.row_stack([[0, 0], np.column_stack([x2, y2])])
            axs.scatter(LGC+0.05,FBC,s=(50+(sum_all/max_total_count)*(500)), \
            c=get_color(EG),marker=xy2,linewidths=0.5)
            prev=prev+norp_lst2[index-1]
#=============================================================================
# SECOND CASE: Single pie chart-------------------------------------------------
    ...
    # create 2 pie for top_pair
        ...
        axs.scatter(LGC-0.05,FBC+0.05,s=(50+(sum_all/max_total_count)*(500)),\
        c=get_color(TP_A2),marker=xy1,edgecolor='black',linewidths=0.8)
        x1 = np.sin(2 * np.pi * np.linspace(top_pair_norp, top_pair_norp))
        y1 = np.cos(2 * np.pi * np.linspace(top_pair_norp, top_pair_norp))
        xy1 = np.row_stack([[0, 0], np.column_stack([x1, y1])])
        axs.scatter(LGC+0.05,FBC+0.05,s=(50+(sum_all/max_total_count)*(500)),\
        c=get_color(TP_A1),marker=xy1,edgecolor='black',linewidths=0.8)
        prev=top_pair_norp
        ...
        c=get_color(TP_A2),marker=xy1,edgecolor='black',linewidths=0.8)
        prev=top_pair_norp
    # create a very big pie first (from x to (1-x%))
        # Need to use this way, otherwise the pie plot will be very ugly.
        x1 = np.sin(2 * np.pi * np.linspace(top_pair_norp, 1-top_pair_norp))
        y1 = np.cos(2 * np.pi * np.linspace(top_pair_norp, 1-top_pair_norp))
        xy1 = np.row_stack([[0, 0], np.column_stack([x1, y1])])
        try: #<- incase of no leaving group (before)
             EG=unique_lst[0]
        except:
             continue
        axs.scatter(LGC,FBC,s=(50+(sum_all/max_total_count)*(500)),\
        c=get_color(EG),marker=xy1,linewidths=0.5)
        prev=top_pair_norp
    #Plot the rest part
        for index,EG in enumerate(unique_lst):
            if index==0:continue # <- we already draw this one.
            x = np.sin(2 * np.pi * np.linspace(top_pair_norp, 1-norp_lst[index-1]-prev))
            y = np.cos(2 * np.pi * np.linspace(top_pair_norp, 1-norp_lst[index-1]-prev))
            xy = np.row_stack([[0, 0], np.column_stack([x, y])])
            axs.scatter(LGC,FBC,s=(50+(sum_all/max_total_count)*(500)),\
            c=get_color(EG),marker=xy,linewidths=0.5)
            prev=prev+norp_lst[index-1]
        # anotation part
        axs.annotate(str(total_count)+' ('+str(total_ref)+')',
                    (LGC, FBC),
                    textcoords="offset points", # how to position the text
        ...
        if max_yield>0:
            axs.annotate(str(max_yield)+' %',
                       ...
                       ha='center')
    #break #<-FOR GING
#cluster_data['EGs']=unique_lst_all
#cluster_data['EGs_nor']=nor_lst_all
#cluster_data['EGs_norp']=norp_lst_all

#plt.show()

fig.tight_layout()
fig.savefig('2_plot.png', dpi=600)
