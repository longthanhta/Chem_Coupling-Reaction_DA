...
def main():
	print('prepare atom mapped smiles of a reaction and put in here:')
	input_smiles=input()
	reactants_input,products_input=input_smiles.split('>>')
	smiles_input = {
			"reactants": reactants_input,
			"products": products_input
			}
	template=extract_from_reaction(smiles_input)['template']
	print('___template:_____________\n',template)
	Draw_temp(template,'template.png')
...
#============================================[Template extractor from Connor Corley]======================================================================
VERBOSE = False
...
def extract_from_reaction(reaction):
	...
	try:
		...
		reactant_fragments, intra_only, dimer_only = get_fragments_for_changed_atoms(reactants, changed_atom_tags,
			radius = 2, expansion = [], category = 'reactants')
...
#		'reactants': reactants_string,
		 'template':products_string+'>>'+reactants_string
...
#==========================[ Fixing template extracted, due to some additional molecule from atom-mapping tool to keep reaciton balance]==================
def template_fix(reaction):
	print('template',reaction)
	LHS,RHS=reaction.split('>>')
	LHS_list=LHS.split('.')
	LHS_fix=[]
	redudant_map=[]	# list of map of atom that been remove from reaction ( those atom are added atom by atom-mapping tool)
	for item in LHS_list:
		if len(item) < 16:
			redudant_map.append(int(re.findall(r'\d+', item[-3:])[0])) # Identify those redudant atom
		else:
			LHS_fix.append(item)
	LHS_fix='.'.join(LHS_fix)
	RHS_fix=[]
	unmap=False
	map_positions=[]   # Get index of map character in the string
	for i in range(0,len(RHS)):
		if RHS[i] == ':':
			map_positions.append(i)
	remove_position=[]
	for index in map_positions:
		cut=RHS[index:index+3]
		big_cut=RHS[index-1:index+1]
		big_cut_2=RHS[index-11:index+1]
		print(big_cut)
		print(big_cut_2)
		atom_map=(int(re.search(r'\d+', cut).group(0)))
		try:
			add_info=(int(re.search(r'\d+', big_cut).group(0)))
		except:
			add_info=1
		if atom_map in redudant_map:
			if len(str(atom_map)) == 1:
				remove_position.append(index)
				remove_position.append(index+1)
			if len(str(atom_map)) == 2:
				remove_position.append(index)
				remove_position.append(index+1)
				remove_position.append(index+2)
			if add_info == 0 and 'H0' in big_cut_2:
				for i in range (1,10):
					remove_position.append(index-i)
			else:
				for i in range (1,7):
					remove_position.append(index-i)
	RHS_fix=[]
	for i in range(0,len(RHS)):
		if i not in remove_position:
			RHS_fix.append(RHS[i])
	RHS_fix=''.join(RHS_fix)

#	After this tep, the mapping is not resionable (for eg, 1,3,5,9). Those number need to be fixed

	map_positions=[]
	map_list=[]
	for i in range(0,len(RHS_fix)):
		if RHS_fix[i] == ':':
			map_positions.append(i+1)
			map_list.append(RHS_fix[i])
	for i in range(0,len(map_positions)):
		for j in range(i,len(map_positions)):
			if map_list[j] < map_list[i]:
				switch=map_positions[i]
				map_positions[i]=map_positions[j]
				map_positions[j]=switch
	RHS_final = list(RHS_fix)
	for i in range(0,len(map_positions)):
		RHS_final[int(map_positions[i])]=str(i+1)
	RHS_final="".join(RHS_final)

# Why only need RHS ? After this step, the reaction will be flip. And LHS will be come RHS.
# !!! Need to apply this function two time: Function(Function(input)).

	return (RHS_fix+'>>'+LHS_fix)
