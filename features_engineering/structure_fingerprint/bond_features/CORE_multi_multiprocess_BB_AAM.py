...
def do_task(output_list,data_file,index):
        ...
        prd_formed_list
        changed_bond_list
        rct_broken_list=cleaning(CORE.print_str(CORE.remove_AM(rct_broken_list)))
        prd_formed_list=cleaning(CORE.print_str(CORE.remove_AM(prd_formed_list,remove_CH_bond=True)),remove_AM=True)
...
def main():
    data_file=pd.read_excel('input.xlsx').fillna('')
    ...
    pool.join()
    #print(output_list[0])
    extracted_df=pd.DataFrame.from_records(output_list,columns=['index','BREAK','LEAVE','CHANGE','FORM_noH','FORM_count'],index='index')
    extracted_df.sort_index()
    out_df=pd.concat([data_file, extracted_df], axis=1)
    out_df.to_excel('output1.xlsx',index=None)
